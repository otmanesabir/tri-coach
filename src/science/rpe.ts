...
export const SESSION_RPE: RPELevel[] = [
...
export const BORG_SCALE: RPELevel[] = [
 ...
 */
export function sessionTrainingLoad(rpe: number, durationMinutes: number): number {
  return rpe * durationMinutes;
}

export function formatRPEScale(): string {
  const lines: string[] = [];
  ...
  for (const level of SESSION_RPE) {
    lines.push(
      `  ${String(level.value).padEnd(3)}  ${level.label.padEnd(15)}  ${(level.hrZoneApprox || '-').padEnd(9)}  ${level.description}`
    );
  }
