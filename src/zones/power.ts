...
export function sweetSpotRange(ftp: number): { min: number; max: number } {
  return {
...
export function formatPowerZones(zones: PowerZone[]): string {
  const lines: string[] = [];
  ...
  for (const z of zones) {
    ...
    lines.push(
      `  ${String(z.zone).padEnd(4)}  ${z.name.padEnd(14)}  ${wStr.padEnd(13)}  ${pctStr.padEnd(8)}  ${z.description}`
    );
  }
  return lines.join('\n');
}
