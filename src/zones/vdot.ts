...
import type { PaceZone } from '../types.js';
 ...
 */
function fractionVO2max(durationMinutes: number): number {
  return (
    0.8 +
 ...
 */
function paceForVdotAtFraction(vdot: number, fractionOfVO2max: number): number {
 ...
 */
export function vdotFromRace(distance: string, time: string): number {
  const meters = RACE_DISTANCES[distance];
  if (!meters) throw new Error(`Unknown race distance: ${distance}. Use: ${Object.keys(RACE_DISTANCES).join(', ')}`);
...
export function calculateDanielsPaces(vdot: number): DanielsPaces {
  return {
 ...
 */
export function predictRaceTimes(vdot: number): Record<string, string> {
...
export function formatDanielsPaces(paces: DanielsPaces): string {
  const lines: string[] = [];
  lines.push(`\n  VDOT: ${paces.vdot}\n`);
  ...
  for (const z of zones) {
    ...
    lines.push(
      `  ${z.zone.padEnd(11)}  ${z.name.padEnd(12)}  ${paceStr.padEnd(18)}  ${z.purpose}`
    );
  }
  return lines.join('\n');
}
