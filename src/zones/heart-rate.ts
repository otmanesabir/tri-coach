...
import type { HeartRateZone, ZoneRange } from '../types.js';

// ──────────────────────────────────────────────
// Friel 7-Zone System (based on LTHR)
// ──────────────────────────────────────────────

interface FrielZoneDef {
  zone: number;
  name: string;
  minPct: number;
  maxPct: number;
...
const FRIEL_ZONES: FrielZoneDef[] = [
...
export function calculateFrielZones(lthr: number): HeartRateZone[] {
...
const HRMAX_ZONES: HRmaxZoneDef[] = [
...
export function calculateHRmaxZones(maxHr: number): HeartRateZone[] {
...
const KARVONEN_ZONES: { zone: number; name: string; minPct: number; maxPct: number; description: string }[] = [
...
export function calculateKarvonenZones(maxHr: number, restHr: number): HeartRateZone[] {
...
export function formatHRZones(zones: HeartRateZone[], method: string): string {
  const lines: string[] = [];
  ...
  for (const z of zones) {
    ...
    lines.push(
      `  ${String(z.zone).padEnd(4)}  ${z.name.padEnd(14)}  ${hrStr.padEnd(13)}  ${pctStr.padEnd(8)}  ${z.description}`
    );
  }
  return lines.join('\n');
}
