...
templates
  ...
  .action((opts: { sport?: string; pretty?: boolean }) => {
...
templates
  ...
  .option('--pretty', 'Human-readable output instead of JSON')
  .action((id: string, opts: { pretty?: boolean }) => {
  ...
  .action((opts: {
    ...
    if (opts.lthr) {
      ...
      if (isNaN(lthr) || lthr < 100 || lthr > 220) {
        ...
        if (opts.pretty) console.log(chalk.red(err));
        else console.log(JSON.stringify({ error: err }));
        return;
      }
