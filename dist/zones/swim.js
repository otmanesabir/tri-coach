 ...
 */
export function calculateCSS(time400Seconds, time200Seconds) {
 ...
 */
export function parseSwimTime(time) {
 ...
 */
export function formatSwimPace(seconds) {
...
export function calculateSwimZones(cssSecPer100m) {
...
export function formatSwimZones(zones, css) {
    const lines = [];
    ...
    for (const z of zones) {
        ...
        lines.push(`  ${String(z.zone).padEnd(4)}  ${z.name.padEnd(10)}  ${paceStr.padEnd(15)}  ${z.cssOffset.padEnd(12)}  ${z.description}`);
    }
    return lines.join('\n');
}
